...
║  + Streaming + Highlighting + Code-Run       ║
║  + Gedächtnis + Themes + YouTube + PDF       ║
╚═══════════════════════════════════════════════╝
...
import subprocess, sys, tempfile, io, urllib.parse, html
...
CODE_FOLDER     = "lern_code"          # Code-Dateien zum Lernen
...
APIS_FILE       = "geist_apis.json"       # Optionale API-Keys
PROFILES_FILE   = "geist_profile.json"    # KI-Profile
TODOS_FILE      = "geist_todos.json"      # Aufgabenliste
...
class GeistApp(ctk.CTk):
    ...
    COMMANDS = {
        ...
        "/model":           "🤖  /model <name>           — KI-Modell wechseln",
    ...
    def _query_ollama(self, user_text, img_b64):
        ...
        try:
            ...
            if has_image:
                ...
                sys_prompt = img_sys
                temp = 0.1   # sehr niedrig für Bilder → weniger halluzinieren
                top_p = 0.8
            else:
                sys_prompt = self._build_system_prompt()
            ...
            resp = requests.post(
                f"{OLLAMA_URL}/api/chat",
                json={"model": cur_model,
                      "messages": msgs,
                      "stream": True,
                      "options": {
                          "temperature": temp,
                          "top_p": top_p,
                          "repeat_penalty": 1.15,   # verhindert Wiederholungen
                          "num_predict": 400,        # max Tokens begrenzen
                      }},
                timeout=300,
                stream=True)
            resp.raise_for_status()
